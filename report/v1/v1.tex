...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Behavioural-cue leakage.} \texttt{infection.py} emits explicit
        ...
        trajectories. v2 replaces literal cues with noisy, false-positive-prone
        hints.
...
the simplified reward hook (one model action then random rollout); v2
addresses this via multi-step model rollouts and an additive action-space
upgrade that lets the v1 LoRA be evaluated zero-shot for transfer measurement.
