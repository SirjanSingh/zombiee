...
\subsection{Extended-Training Corroboration}
\label{sec:extended}
...
Headline parity holds: survival rate $0\,\%\rightarrow 12\,\%$ at the final
checkpoint (vs.\ $0\,\%\rightarrow 10\,\%$ for the v1 step-12 eval), and
vote accuracy $20\,\%$ on episodes that reached the vote phase (the v1
eval saw only a single vote fire across $n_t{=}10$, so its vote accuracy
is effectively undefined). The extended run did not export a numeric
eval JSON, so we restrict the report's quantitative claims (Table~\ref{tab:metrics},
Figures~\ref{fig:bars}--\ref{fig:history}) to the v1 step-12 eval, which
remains the canonical evaluation of record.

The clearest signal from the extended run is the per-step suspicion
trajectory in Figure~\ref{fig:infected}. The trained policy's mean
suspicion on the true infected agent rises from chance (${\sim}0.33$ for
a uniform 1-of-3 pick) at $t{=}1$ to near-certainty (${\sim}1.0$) by
$t{\approx}80$, with the lift starting cleanly at the $t{=}30$ infection
reveal and continuing past the $t{=}50$ vote. The uniform-random baseline
hovers around $0.33$ throughout. This is direct evidence of the hidden-role
theory-of-mind signal the env was designed to elicit, and the metric is
not measurable on the v1 step-12 eval (which terminates too early in most
episodes for the post-reveal portion of the trajectory to develop).

\begin{figure}[h]
  \centering
  \includegraphics[width=0.92\textwidth]{figures/infected_detection.png}
  \caption{Per-step mean suspicion on the true infected agent across an
           episode (extended-run eval). Trained policy (blue) climbs from
           ${\sim}0.33$ (chance) to ${\sim}1.0$; uniform-random baseline
           (orange) stays at chance. Vertical markers: red dashed = infection
           reveal at $t{=}30$, green dashed = vote at $t{=}50$. Source:
           \texttt{noanya/zombiee-v1-extended/eval\_results/infected\_detection.png}.}
  \label{fig:infected}
\end{figure}

The $20\,\%$ vote accuracy sits below the $33\,\%$ uniform-pick floor for
a 1-of-3 categorical choice, which is consistent with the suspicion
trajectory: at $t{=}50$ the mean suspicion on the true infected has only
just crossed ${\sim}0.55$, so the categorical commitment demanded by the
single $t{=}50$ vote is premature relative to where the soft posterior
becomes reliable (${\sim}0.9$ around $t{=}70$). A natural follow-up
(see §\ref{sec:future}) is to delay the vote or run iterated votes so
the categorical decision lands after the signal stabilises. The Hugging
Face Space serves the extended-run adapter; the report's quantitative
results are anchored on the v1 step-12 adapter for the reasons noted above.
...
The biggest constraint shaping every other limitation below is compute. The
entire pipeline --- env design, GRPO training, and LLM-driven evaluation ---
ran on a single free-tier Colab Tesla T4 (15.6\,GB, no native bf16) inside
the 24-hour hackathon window. The training run itself consumed
$\approx$3\,h\,53\,min for 12 GRPO steps ($\approx$19\,min/step), and the
LLM-driven evaluation costs $\approx$3\,min per trained episode. Several
extensions we would have liked to ship were straightforwardly bottlenecked
by this budget rather than by design.

\begin{itemize}[leftmargin=*]
  \item \textbf{Compute budget.} A single T4 with $\approx$19\,min/step on
        Qwen-3B + LoRA limited training to 12 GRPO steps within the
        hackathon window. KL drift at step 12 was $<5\!\times\!10^{-3}$ ---
        i.e.\ the policy had not yet fully converged. A longer run
        (50--100 steps) would likely have widened the survival-rate gap
        further, but requires either an A100/H100 (native bf16, $\approx$2$\times$
        throughput) or a multi-day GPU lease, neither of which was available
  ...
  \item \textbf{Reward-signal weakness.} The reward hook scores only the
        ...
        Multi-step model rollouts would tighten this signal but multiply
        per-episode compute by 10--20$\times$, which compounds the budget
        problem above.
  ...
  \item \textbf{Sample size.} $n_t{=}10$ gives a 95\% binomial CI for the
        ...
        headline. A larger eval ($n_t{=}50$) at $\approx$3\,min per
        LLM-driven episode is another 2.5\,h of T4 time on top of the
        training run --- again a compute, not a design, constraint.
...
with no breaking changes to the action space or reward interface. The
directions below are scoped as future work rather than ablations on the
current submission because each requires substantially more GRPO compute
than fit inside the 24-hour hackathon window on a single T4 (see
§\ref{sec:limitations}); on an A100/H100 with native bf16 we estimate
each one to be a 1--2 day extension rather than a redesign.
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item \textbf{Iterated voting.} Vote phases at $t\in\{30,60,90\}$ instead
        ...
        The extended-run suspicion trajectory (§\ref{sec:extended},
        Figure~\ref{fig:infected}) supports this directly: at $t{=}50$ the
        soft posterior on the true infected has only just crossed ${\sim}0.55$,
        whereas by $t{=}70{-}80$ it has reached ${\sim}1.0$. A late vote
        (or an iterated one) would land after the signal has stabilised.
  ...
  \item \textbf{Noisy behavioural cues.} Replace the literal-string cue
        ...
        match strings.
  \item \textbf{Multi-step model rollouts in the reward hook.} The current
        hook scores only the model's first action and rolls the rest of the
        episode out with random actions; a clean fix is to keep the model in
        the loop for $K$ steps before random rollout. Each
        $K{=}10$ increment scales reward-fn compute by $\approx$10$\times$
        (the dominant cost per GRPO step), which is why this is
        compute-gated rather than implementation-gated.
